...
if (( ${#window_status_sub_elements[@]} )); then
  tmux set -ga window-status-format "#[fg=$TMUX_CZ_LIGHT_GRAY,bg=default,reverse]$left_separator_glyph#[noreverse]#[none]"
fi

for (( i = 0; i < ${#window_status_sub_elements[@]}; i++ )); do
  if (( i != 0 )); then
    tmux set -ga window-status-format "#[fg=$TMUX_CZ_LIGHT_GRAY,bg=default,reverse]$left_subseparator_glyph#[noreverse]#[none]"
  fi
...
if (( ${#window_status_sub_elements[@]} && ${#window_status_main_elements[@]} )); then
  ...
  tmux set -ga window-status-format "#[fg=$TMUX_CZ_DARK_GRAY,bg=default,reverse]$left_separator_glyph#[noreverse]#[none]"
fi

for (( i = 0; i < ${#window_status_main_elements[@]}; i++ )); do
  if (( i != 0 )); then
    tmux set -ga window-status-format "#[fg=$TMUX_CZ_DARK_GRAY,bg=default,reverse]$left_subseparator_glyph#[noreverse]#[none]"
  fi
...
if (( ${#window_status_current_sub_elements[@]} )); then
  tmux set -ga window-status-current-format "#[fg=$TMUX_CZ_YELLOW_GREEN,bg=default,reverse]$left_separator_glyph#[noreverse]#[none]"
fi

for (( i = 0; i < ${#window_status_current_sub_elements[@]}; i++ )); do
  if (( i != 0 )); then
    tmux set -ga window-status-current-format "#[fg=$TMUX_CZ_YELLOW_GREEN,bg=default,reverse]$left_subseparator_glyph#[noreverse]#[none]"
  fi
...
if (( ${#window_status_current_sub_elements[@]} && ${#window_status_current_main_elements[@]} )); then
  ...
  tmux set -ga window-status-current-format "#[fg=$TMUX_CZ_NAVAJO_WHITE,bg=default,reverse]$left_separator_glyph#[noreverse]#[none]"
fi

for (( i = 0; i < ${#window_status_current_main_elements[@]}; i++ )); do
  if (( i != 0 )); then
    tmux set -ga window-status-current-format "#[fg=$TMUX_CZ_NAVAJO_WHITE,bg=default,reverse]$left_subseparator_glyph#[noreverse]#[none]"
  fi
...
for (( i = 0; i < ${#status_right_elements[@]}; i++ )); do
  ...
  case "${status_right_elements[i]}" in
    TMUX_CZ_RIGHT_DECORATION )
      ...
      ;;
    * )
      if (( i != 0 )) && [[ ${status_right_elements[i - 1]} != TMUX_CZ_RIGHT_DECORATION ]]; then
